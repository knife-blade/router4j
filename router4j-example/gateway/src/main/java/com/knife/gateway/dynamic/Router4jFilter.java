...
public class Router4jFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 修改uri
        URI newUri = UriComponentsBuilder.fromHttpUrl(hostAndPort + rawPath)
                .build()
                .toUri();

        //重新封装request对象
        ServerHttpRequest newRequest = req.mutate().uri(newUri).build();
    ...
    //     URI mergedUri = UriComponentsBuilder.fromUri(uri)
